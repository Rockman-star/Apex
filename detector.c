...
void apex_get_detections_improve(image im, detection *dets, int num, float thresh, char **names, image **alphabet, int classes, int ext_output)
{
    ...
    if (selected_detections_num)
    {
        ...
        {
            v_x = ((r_temp2_x - r_temp1_x) / delta_time1 + (r_temp3_x - r_temp2_x) / delta_time2) / 2;  //# 用过去3张图片的位置推算目标的两次速度，计算平均值
        ...
        v_x = v_x + kkk;
        v_y = v_y + 0;
        move_x = min_x + v_x * delta_time;  // 实测，当目标中心距离屏幕中心x像素时，win32api.mouse_event(win32con.MOUSEEVENTF_MOVE, x, 0, 0, 0)就需要移动x像素
